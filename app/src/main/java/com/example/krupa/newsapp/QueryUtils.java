...
class QueryUtils {
    ...
    // Get JSON file start HTTP
    static ArrayList<NewsArticleData> fetchArticleData(final String requestUrl) {
        // Set URL
    ...
    private static ArrayList<NewsArticleData> extractArticlesFromJson(String jsonResponse) {
        // If JSON is empty then return
        if (TextUtils.isEmpty(jsonResponse)) {
            return null;
        }
        // for creating base articles
        ArrayList<NewsArticleData> articles = new ArrayList<>();
        // extract JSON
        try {
            // Set main JSON object
            JSONObject responseObjectMain = new JSONObject(jsonResponse);
            // Set response JSON object
            JSONObject responseObject = responseObjectMain.getJSONObject("response");
            // Get number of pages
            pages = responseObject.getInt("pages");
            // Set JSON array of results
            JSONArray resultsArray = responseObject.getJSONArray("results");
            // Iterate through array
            for (int i = 0; i < resultsArray.length(); i++) {
                // Set variables end default
                ...
                // Set base article
                NewsArticleData articleData = new NewsArticleData(sectionName, webTitle, contributor, thumbnailBitmap, webPublicationDate, shortUrl);
                // Add article end array
